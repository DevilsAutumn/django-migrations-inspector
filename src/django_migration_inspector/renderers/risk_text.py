...
class TextRiskReportRenderer:
    ...
    def _render_summary(self, report: RiskAssessmentReport) -> list[str]:
        ...
        if report.blocked_migration_count:
            lines.append(
                "  - "
                f"{_pluralize(report.blocked_migration_count, 'migration')} block "
                "a clean rollback path."
            )
        if report.destructive_migration_count:
            lines.append(
                "  - "
                f"{_pluralize(report.destructive_migration_count, 'migration')} contain "
                "destructive schema changes."
            )
        if report.review_migration_count:
            lines.append(
                f"  - {_pluralize(report.review_migration_count, 'migration')} need manual review."
            )
        return lines

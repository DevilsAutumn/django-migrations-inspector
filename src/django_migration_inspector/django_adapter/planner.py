...
def _build_target_leaf_nodes(
    *, executor: MigrationExecutor, app_label: str | None
) -> tuple[MigrationNodeKey, ...]:
    if app_label is None:
        selected_targets = tuple(sorted(executor.loader.graph.leaf_nodes()))
    else:
        if not any(raw_key[0] == app_label for raw_key in executor.loader.disk_migrations):
            raise MigrationInspectionError(
                f"App {app_label!r} has no migrations in the loaded Django project."
            )
        selected_targets = tuple(sorted(executor.loader.graph.leaf_nodes(app_label)))

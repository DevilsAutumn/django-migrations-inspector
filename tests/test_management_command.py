...
def test_management_command_fails_on_multiple_heads_after_rendering_json(
    ...
    with django_db_blocker.unblock():
        try:
            call_command("migration_inspect", "--json", "--fail-on-multiple-heads", stdout=output)
        except CommandError as error:
            error_message = str(error)
        else:
            raise AssertionError("Expected multiple-head policy to fail.")

    report = json.loads(output.getvalue())
    assert report["report_type"] == "graph_inspection"
    assert report["multiple_head_apps"][0]["app_label"] == "analytics"
    assert "multiple migration heads found in analytics" in error_message
...
def test_management_command_allows_multiple_heads_policy_when_graph_is_clear(
    django_db_blocker: DjangoDbBlocker,
) -> None:
    output = StringIO()
    ...
    with django_db_blocker.unblock():
        call_command(
            "migration_inspect", "--app", "inventory", "--fail-on-multiple-heads", stdout=output
        )

    assert "Decision: CLEAR" in output.getvalue()


def test_management_command_rejects_severity_policy_for_inspect() -> None:
    output = StringIO()

    try:
        call_command("migration_inspect", "--fail-on-severity", "high", stdout=output)
    except CommandError as error:
        assert "--fail-on-severity is available only for risk, audit, and rollback modes" in str(
            error
        )
    else:
        raise AssertionError("Expected inspect severity policy to fail.")
